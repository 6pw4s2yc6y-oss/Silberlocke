...
> Stand 2026-07-11 (Abend): **451/451 Logik-Tests grün**, 48 Logik-Module (`src/logic/`), 35 Screens (`src/screens/`). Seit dem Vormittags-Snapshot (431 Tests, 46 Module, 33 Screens) neu dazugekommen: Survival Mode (#160, PR #43) · Versions-Wasserzeichen seit der PWA fortgesetzt (PR #45, aktuell v104) · Shadow-Mode-Stufen-Umbau nach Betreiber-Feedback per Screen-Recording (PR #44): eigene garantierte 14-Tage-Dauer (`SHADOW_MODE_DAYS`, unabhängig von `DAYS_PER_STAGE`), rückwärts laufende Atomuhr (echter Countdown statt vorwärts-elapsed) während Shadow, minimale MasterScreen-Ansicht in Shadow (kein Werkzeuge-/Shop-/Manifest-Zugriff), Kennzahlen-Leiste als Wisch-Karussell statt dichtem Grid (`StatCarousel`, Haptik pro Seitenwechsel), und der alte 5-Schritte-Onboarding-Wizard komplett ersetzt durch Drip-Fragen alle 2 Tage (`shadowOnboarding.ts`, Tag 0/2/4/6/8/10/12) plus wiederkehrendem Waage-Tracker. Der frühere „Session 2 / 65 Tests"-Block ist damit mehrfach überholt — der reale Stand ist deutlich weiter.
