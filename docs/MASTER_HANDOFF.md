...
*   **`vaaav-mobile`** ist das **aktive Entwicklungs-Repo** (Expo SDK 54/57, React Native 0.81, TypeScript, react-native-reanimated 4). Stand 2026-07-11, direkt gegen den Code geprüft: **431/431 Logik-Tests grün** (`npm test`), **46 Logik-Module** in `src/logic/`, **33 Screens** in `src/screens/`, Core Bar + Theme-Kontext + Onboarding vorhanden. **Der reale Fortschritt liegt weit über dem alten Session-2-Statusblock** (siehe 0.3) — siehe 7B für den vollen 2026-07-11-Änderungslog. CI (`.github/workflows/ci.yml`): Typecheck + Logik-Tests bei jedem Push; Auto-Publish via `eas-update.yml`.
...
*   (131) ✅ Regressions-Frühwarnsystem in `vaaav-mobile` (PR #19): vergleicht den Mood-Schnitt der letzten 7 Tagebuch-Einträge (#148) mit dem Schnitt der 7 davor, erst ab 4 Einträgen je Fenster. Bewusst NICHT als Diagnose formuliert (kein „Depressions"-Wording im Code/UI) — das System erkennt ein Muster, keine Krankheit, gleicher Grundsatz wie der Disclaimer bei #110. Karte „Sanfter Hinweis" auf MasterScreen. 7 Tests, 386/386 grün. (`regressionWatch.ts`, MasterScreen)
...
*   (139) ✅ Thermodynamisches Recovery-Fenster in `vaaav-mobile` (PR #17): roter Defizit-Modus füllt sich nur aktiv über echte, im DayScreen getrackte Kcal (`addRecoveryCredit`), nie passiv durch Zeit. Die Logik existierte schon (`recoveryDebt`/`recoveryRecovered` in `discipline.ts`), aber RecoveryModeScreen zeigte bis zu diesem Fix eine davon losgelöste Mock-Anzeige (fester Fake-Wert statt echtem Fortschritt) — jetzt korrekt verdrahtet, inklusive Rehydrierung über den echten Gläser-Zähler (`addWater`) statt einer zweiten, unverbundenen ml-Eingabe. Screen-only Fix, 371/371 Tests weiterhin grün.
...
*   (141) ✅ Mikrozyklen-Spezifizierung in `vaaav-mobile` (PR #22): Trainingsreiz-Abfrage je Trainingstag (Kraftausdauer/Hypertrophie/Muskelausdauer) direkt unter der Tagestyp-Wahl im DayScreen; derselbe Reiz zwei Kalendertage in Folge löst eine Warnung aus (physiologisch widersprüchliches Muster, Manifest #16). Bewusst Warnung statt harter Blockade — gleiche „keine Bevormundung"-Konvention wie Hybrid-Routing (#140). Neues `stimulusLog`-Feld in ProgressState. 8 Tests, 401/401 grün. (`microcycle.ts`, DayScreen)
...
*   (51) ✅ (125) ✅ Leistungs-Log in `vaaav-mobile` (PR #20): manuelles Fahrten-Log (Distanz/Dauer/Ø Watt/Höhenmeter, #51 Watt-Tracking). Apex-Leistungen (#125) sind keine separate Eingabe, sondern eine abgeleitete Eigenschaft desselben Logs — die Fahrt mit dem persönlichen Bestwert je Kennzahl bekommt automatisch ein Apex-Badge. Beide Punkte sind laut Handoff „Phase 1: manuelle Eingabe/Übertragung" desselben Datenmodells; Hardware-/BLE- bzw. Strava-Sync bleiben Phase 3. Separater Speicher (`sl_rides`), keine Punkte-Mechanik. 7 Tests, 393/393 grün. (`ridePerformance.ts`, RidePerformanceScreen)
*   (60) ✅ Gesundheits-Metriken in `vaaav-mobile`: Gewicht + Schlafstunden als gemeinsames Tages-Log (Upsert per Datum, `sl_health_metrics`). Schließt zugleich eine echte Lücke — BodyScreen lief bislang mit `mock.body.weights` fürs Thermodynamik-Audit ("Eingabe folgt mit der Persistenz-Schicht" stand seit der ersten Session als offener Kommentar), jetzt echte Ein-/Ausgabe inkl. sauberem Leerzustand. Apple-Health/Health-Connect-Sync bleibt Phase 3. 7 Tests, 393/393 grün. (`healthMetrics.ts`, BodyScreen)
...
*   (148) ✅ Belohnungs-System für strukturierte Tagebuch-Einträge in `vaaav-mobile` (PR #12): +10 Punkte einmal täglich ab 20 Zeichen, kein Farming durch Kurz-Einträge. Legt die Grundlage für #131/#161/#162 (Journal-Mechanismus existierte vorher gar nicht). 7 Tests, 335/335 grün. (`journal.ts`, JournalScreen)
...
*   (160) ✅ Survival Mode in `vaaav-mobile` (PR #43): autarker Modus für mehrtägige Touren abseits der Zivilisation. `survivalModeSince`/`survivalPeriods` in `discipline.ts` pausieren die reguläre Verpasst-Tage-Strafe – auch rückwirkend, falls unterwegs kein App-Zugriff bestand (`reconcileProgress`-Exemption). Eigenes Trip-Log (`survivalMode.ts`, Storage-Key `sl_survival_log`) für Kilometer/Höhenmeter/Nächte im Freien/Rationierung, wie journal.ts/checkins.ts. `SurvivalModeScreen` + Werkzeuge-Wiring. 9 Tests, 440/440 grün.
*   (161) ✅ Tages-Check-ins in `vaaav-mobile` (PR #42): dreimal täglich (Morgen/Mittag/Abend) strukturierte Abfrage von Wohlbefinden (1–5) + optionaler Kurznotiz. Ergänzt das bestehende Tagebuch (#148), bewusst OHNE eigene Punkte-Belohnung (kein Farming-Anreiz durch drei Klicks). Slot-Chips in `JournalScreen` (erledigt/offen, aktueller Slot dezent hervorgehoben), Inline-Editor statt eigenem Screen. 8 Tests, 431/431 grün. (`checkins.ts`, JournalScreen)
...
*   (164) ✅ Biometrische Resilienz-Kopplung in `vaaav-mobile` (PR #28): gleicht ein erkanntes „Tief" (#136) mit physischen Daten ab — frische harte Rad-Einheit (#51, gleiche >100km/3h-Schwelle wie der bestehende RecoveryMode-Trigger) oder kurzer Schlaf-Schnitt (#60). Bei Überlastung wechselt das Wording von Disziplin zu „Erholung ist Teil des Prozesses" (Manifest #16) statt „Push harder". Reine Ableitung, kein neuer Speicher-Key. 8 Tests, 423/423 grün. (`biometricResilience.ts`, MasterScreen)
...
### 7B. VERIFIZIERT: React-Native-App (`vaaav-mobile`, direkt gegen den Code, 2026-07-11)
> Stand 2026-07-15: **451/451 Logik-Tests grün**, 54 Logik-Module (`src/logic/`), 35 Screens (`src/screens/`), Versions-Wasserzeichen bei **v125**. **Kurswechsel (2026-07-14): "Mission Zero"-Governance aktiv.** Der Betreiber hat ein 11-teiliges Prompt-Framework eingeführt, das ab sofort den fachlichen Prozess ersetzt: **Development Freeze** (keine neuen Features/Screens/Designänderungen mehr, bis die Architektur stabil ist), verbindliche Ownership-Tabelle pro Datentyp, vier nicht-verhandelbare Architekturregeln (Controller Pattern, Single Source of Truth, Zod-`safeParse` bei jedem Storage-Read, Error Boundaries) und ein zweistufiges Vorgehen: erst vollständige Bestandsaufnahme (`VAAAV_SYSTEM_TRUTH.md`, alle 47 Logik-Module + 35 Screens + 5 Contexts + 10 Components + `storage.ts` einzeln geprüft, 10 dokumentierte Funde, u. a. Gewicht-Doppelspeicherung zwischen `healthMetrics.ts`/`shadowOnboarding.ts`, ein "God Context" `DisciplineContext.tsx` mit >10 gebündelten Verantwortungen, fehlende Zod-Validierung, Business-Logik direkt in 19 Screens), dann ein vom Betreiber freigegebener ADR (`VAAAV_ADR_001.md`) mit priorisierter, modulweiser Migrationsreihenfolge. **Migrationsschritt 1 – Storage-Fundament (PR #60/#61, v113→v114):** `KEYS`-Export in `storage.ts` von 3 auf alle 42 bekannten Speicherschlüssel erweitert, neue `schemas.ts` mit Zod-Schemas, neue `readJson()`/`readString()`-Helfer (Storage-Read + `safeParse` + Fallback statt reinem `try/catch`); als erster Anwendungsfall auf die Profil-/Wach-/Schlafzeit-Hydration in `ProfileContext.tsx` angewendet – ein inhaltlich beschädigter, aber syntaktisch gültiger Eintrag führt jetzt korrekt zurück zum Onboarding statt stillschweigend übernommen zu werden. Rein additiv, keine anderen Module angefasst. **Vom Betreiber in Expo Go bestätigt** (Mission-Zero-Pflicht: nur das gilt als "live"). **Migrationsschritt 2 – DisciplineContext-Split (PR #62/#63, v114→v115):** löst Fund 3 (VAAAV_SYSTEM_TRUTH.md) – der 363-Zeilen-„God Context" `DisciplineContext.tsx` (bündelte Disziplin-Kern, Wasser, Recovery, Krankheits-Tag, Eternity Mode, DEV-Stage-Wechsel, Mikrozyklus, Quiz, Wissens-Feed, Wallpaper-Bar, Resilienz, Journal-Belohnung, Survival Mode und den Punkte-Shop) ist jetzt in **11 fokussierte Hooks** unter dem neuen `src/features/discipline/hooks/` aufgeteilt (erste Anwendung der Feature-Ordner-Struktur aus dem ADR), alle über einen gemeinsamen internen `progressStore.tsx` auf demselben `sl_progress`-Datensatz – keine Datenmigration. `useDiscipline()` bleibt als Fassade mit identischer Rückgabestruktur bestehen, keine der 18 bestehenden Screens/Komponenten musste angepasst werden. Re-Render-Isolation ist bewusst kein Teil dieses Schritts (erst nach einer echten Performance-Messung). **Vom Betreiber in Expo Go bestätigt.** **Migrationsschritt 3 – DayScreen-Hooks (PR #64/#65, v115→v116):** löst den größten Einzelfall aus Fund 9/10 – `DayScreen.tsx` griff an 8 Stellen direkt auf `store.getItem()`/`setItem()` zu (`sl_daytype`, `sl_identity`, `sl_penalty`, `sl_confessions`, `sl_week`, `sl_weekplan`, `sl_train`, `sl_training_log_<datum>`). Logik 1:1 unverändert in **7 neue Hooks** unter `src/features/day/hooks/` verschoben (`useDayType`, `useIdentityReminder`, `useConfession`, `useWeekPlanClash`, `useSelectedTrainingProgram`, `useTrainTimes`, `useTrainingLogSave`); zwei davon lesen jetzt über `KEYS.week`/`KEYS.weekplan` (storage.ts) statt neuer lokaler Konstanten – ein kleiner Schritt gegen die in Fund 10 dokumentierte Vervierfachung derselben Key-Strings. `DayScreen.tsx` behält seine komplette Render-Logik unverändert. **Vom Betreiber in Expo Go bestätigt.** Damit sind die drei höchstprioritären Funde (Storage-Fundament, DisciplineContext, DayScreen) abgearbeitet. **Migrationsschritt 4 – 8 einfache Screens (PR #66/#67, v116→v117):** erster Teilschritt für Fund 10 (die übrigen 18 Screens mit direktem Storage-Zugriff) — die 8 einfachsten (je ein Speicherschlüssel, simples Lese-/Schreibe-Paar) bekamen jeweils einen eigenen kleinen Hook: `FindingsScreen`→`useFindings` (sl_findings), `DayArchitectureScreen`→`useDayArchitecture` (sl_day_architecture), `DrivetrainScreen`→`useDrivetrainLog` (sl_drivetrain), `MonitoringScreen`→`useMonitorLog` (sl_monitor), `RidePerformanceScreen`→`useRideLog` (sl_rides), `GearingScreen`→`useGearingSetup` (sl_gearing), `SurvivalModeScreen`→`useSurvivalLog` (sl_survival_log), `AnalyticsScreen`→`useWeekPlanRead` (liest jetzt über `KEYS.week` statt einer neuen lokalen Konstante). Alle unter neuen `src/features/<bereich>/hooks/`-Ordnern, Logik 1:1 unverändert verschoben, Render-Logik jedes Screens unangetastet. **Vom Betreiber in Expo Go bestätigt.** **Migrationsschritt 5 – 5 weitere Screens (PR #68/#69, v117→v118):** zweiter Teilschritt für Fund 10 — `InventoryScreen`→`useInventory` (sl_inventory), `BloodworkScreen`→`useBloodwork` (sl_blood, sl_blood_history), `GearSetupScreen`→`useGearSetup` (sl_gear, sl_pretour_checklist), `JournalScreen`→`useJournalEntries` (sl_journal) + `useCheckins` (sl_checkins, bewusst zwei getrennte Hooks für zwei fachlich unabhängige Features #148/#161), `RamadanModeScreen`→`useRamadanSettings` (sl_ramadan_optin/suhoor/iftar). Alle unter neuen `src/features/<bereich>/hooks/`-Ordnern, Logik 1:1 unverändert verschoben. **Vom Betreiber in Expo Go bestätigt.** **Migrationsschritt 6 – WeeklyPlanScreen (PR #70/#71, v118→v119):** dritter Teilschritt für Fund 10, diesmal der wichtigste verbleibende — `WeeklyPlanScreen.tsx` ist der einzige Schreiber von `sl_week`/`sl_weekplan`, die DayScreen/AnalyticsScreen bereits lesend nutzen. Zwei neue Hooks unter `src/features/weeklyPlan/hooks/`: `useWeekSchedule` (Wochenkalender Training/Pause je Tag) und `useWeeklyTrainingPlan` (gewähltes Trainingsprogramm, wählen/zurücksetzen). Per Playwright-Cross-Check verifiziert: eine Änderung in WeeklyPlanScreen wird korrekt von DayScreens Clash-Erkennung erkannt (Widerspruch-Banner erscheint exakt wie erwartet). **Vom Betreiber in Expo Go bestätigt.** **Update-Check-Button (PR #72/#73, v119→v120) — bewusste, vom Betreiber explizit freigegebene Einzel-Ausnahme vom Development Freeze, kein Migrationsschritt:** ein kleiner Info-Button direkt neben dem Versions-Wasserzeichen öffnet ein Modal (`UpdateChecklistModal.tsx`) mit einer kurzen, pro Versions-Bump manuell gepflegten Liste (`data/updateChecklist.ts`), was für die jeweils aktuelle Version in Expo Go zu prüfen ist — Hintergrund: der Betreiber hatte wiederholt eine alte, gecachte Version kontrolliert statt der neuesten; die Liste nennt die geprüfte Versionsnummer explizit, damit das nicht mehr passiert. Umsetzung ohne Datenmigration; einzige Nebenkorrektur war `pointerEvents` am Versions-Badge-Wrapper von `"none"` auf `"box-none"`, damit der neue Button tappbar wird, ohne Touches auf dem restlichen Screen zu blockieren (per Playwright verifiziert). **Vom Betreiber in Expo Go bestätigt.** **Migrationsschritt 7 – MasterScreen (PR #74/#75, v120→v121):** vierter Teilschritt für Fund 10 — `MasterScreen.tsx` griff an 4 Stellen direkt über `store` zu (`sl_journal`, `sl_health_metrics`, `sl_rides`, `sl_shadow_weighins`). Vier neue Hooks unter `src/features/master/hooks/`: `useJournalRegression` (Regressions-Frühwarnsystem #131, rein lesend), `useHealthMetricsLog` (Gewichts-Verlauf, biometrische Überlastungs-Kopplung #164, Schlaf-Tracker – liest + `saveSleep()`), `useMasterRideLog` (rein lesend, bewusst ein eigener kleiner Hook statt Wiederverwendung von `features/ridePerformance/useRideLog`, da jener asynchron über `useEffect` hydriert und MasterScreen bisher synchron beim ersten Render las — eine Wiederverwendung hätte beim Öffnen kurz falsche/leere Werte gezeigt), `useShadowWeighIns` (wiederkehrender Waage-Tracker im Bogen-Panel – liest + `saveWeighIn()`). Logik 1:1 verschoben, `store`-Import komplett aus dem Screen entfernt. Bewusst **keine** Vereinheitlichung von Fund 1 (Gewicht-Doppelspeicherung zwischen `sl_health_metrics`/`sl_shadow_weighins`) in diesem Schritt — bleibt separater, künftig einzeln geplanter Fix (Trennregel). Per Playwright verifiziert: Gewichts-/Schlaf-Anzeige und -Eintrag unverändert, insbesondere kein Aufblitzen leerer Werte beim Öffnen der Übersicht (Grund für den eigenen Hook statt Wiederverwendung). **Vom Betreiber in Expo Go bestätigt.** **Migrationsschritt 8 – MoneyScreen (PR #76/#77, v121→v122):** fünfter Teilschritt für Fund 10 — `MoneyScreen.tsx` verwaltete Einkommen, Fixkosten/Schulden, Budget-Aufteilung und Trash-Ausgaben komplett direkt über `store` unter der lokalen Konstante `MONEY_KEY`, obwohl `KEYS.money` bereits seit Migrationsschritt 1 existiert. Ein neuer Hook `useMoneyData()` unter `src/features/money/hooks/` liest/schreibt `sl_money` jetzt über `KEYS.money` (löst nebenbei die Key-Dopplung) und liefert `data`, `loaded` sowie die 8 Aktionen 1:1 (`addIncome`, `removeIncome`, `addCost`, `removeCost`, `setBudgetSupps`, `setBudgetFood`, `addTrash`, `removeTrash`), inkl. der bestehenden No-Op-Guards. Der neue `loaded`-Flag sorgt dafür, dass die lokalen Anzeige-Strings für die Budget-Eingabe weiterhin nur einmalig beim Laden gesetzt werden statt bei jedem Update, damit beim Tippen kein Komma verschluckt wird. `store`-Import komplett aus dem Screen entfernt. **Vom Betreiber in Expo Go bestätigt.** **Migrationsschritt 9 – RecoveryModeScreen (PR #78/#79, v122→v123):** letzter Teilschritt für Fund 10 — `RecoveryModeScreen.tsx` schrieb an einer Stelle direkt über `store`: `saveSession()` hängte einen Eintrag an das qualitative Recovery-Journal (`sl_recovery_log`) an, unter der lokalen Konstante `RECOVERY_LOG_KEY` statt des bereits existierenden `KEYS.recoveryLog`. Ein neuer, sehr kleiner Hook `useRecoveryLog()` unter `src/features/recoveryMode/hooks/` übernimmt die reine Schreib-Aktion (`addSession`) jetzt über `KEYS.recoveryLog` (löst die Key-Dopplung); die Guards und die Session-Zusammensetzung bleiben bewusst im Screen, da sie von screen-lokalem State (`inRecovery`, `waterCount` aus `useDiscipline()`) abhängen. `store`-Import komplett aus dem Screen entfernt. **Vom Betreiber in Expo Go bestätigt.** **Damit ist Fund 10 (verstreuter direkter Storage-Zugriff in Screens) nach 9 Migrationsschritten vollständig abgearbeitet** — kein Screen greift mehr direkt auf `store` zu, jede Storage-Interaktion läuft über einen dedizierten Feature-Hook. **Migrationsschritt 10 – Fund 4: FinMode aus ProfileContext gelöst (PR #80/#81, v123→v124):** `ProfileContext.tsx` bündelte fünf fachlich unterschiedliche Bereiche (Onboarding-Status, Profil + Tagesrhythmus, FinMode, Fokus-Matrix, Drip-Onboarding-Antwortstatus) — FinMode (König/Warrior-Umschalter) gehört laut Ownership-Tabelle fachlich zur Money Engine, nicht zum Profil. Neuer, eigenständiger Context `src/state/FinModeContext.tsx` (schlankes Muster wie `ArcModeContext.tsx`/`StackContext.tsx`) übernimmt `finMode`/`toggleFinMode` jetzt über `KEYS.finMode` statt der bisherigen lokalen `FINMODE_KEY`-Konstante (löst die Key-Dopplung). Neu ergänzt: `resetFinMode()` — kein zusätzliches Feature, sondern notwendig, um das bisherige Verhalten von `ProfileContext.reset()` (setzte `finMode` bisher mit zurück) über die beiden jetzt getrennten Contexts hinweg 1:1 zu erhalten; `ToolsScreen.tsx`s Reset-Handler ruft künftig beide Reset-Funktionen auf. **Vom Betreiber in Expo Go bestätigt.** Damit ist Fund 4 abgeschlossen. **Migrationsschritt 11 – Fund 5: discipline.ts in 7 fachliche Module aufgeteilt (PR #82/#83, v124→v125):** `discipline.ts` (638 Zeilen) bündelte acht fachlich unterscheidbare Bereiche in einer Datei. Logik 1:1 in sieben neue, fokussierte Dateien unter `src/logic/` verschoben: `disciplineCore.ts` (Stage/Score-Kern, Freischalt-Fahrplan/Gatekeeper, Degradierungs-Automatik, `ProgressState` + Sub-Interfaces), `hydration.ts`, `sickDay.ts`, `survivalStatus.ts` (bewusst **nicht** `survivalMode.ts` genannt — kollidiert sonst mit dem bereits existierenden Reise-Metriken-Modul, im ADR selbst als offene Namensfrage vermerkt), `eternityMode.ts`, `pointsShop.ts`, `recoveryDebt.ts` (ein zusätzlicher, klar abgrenzbarer neunter Bereich, in der ursprünglichen Fund-5-Liste nicht separat benannt). `discipline.ts` selbst ist jetzt eine reine Re-Export-Fassade — **keine der 32 Konsumenten-Dateien und keine der 19+ Testdateien musste angepasst werden**, da alle bisherigen Namen unverändert weiter exportiert werden. Bekannte, bewusst akzeptierte zirkuläre Modul-Abhängigkeit: `disciplineCore.ts` (`reconcileProgress`) importiert `isSurvivalDate` aus `survivalStatus.ts`, während `survivalStatus.ts` umgekehrt `evaluateDay` aus `disciplineCore.ts` braucht — bei reinen Funktions-Exports ohne Seiteneffekte beim Modul-Laden unproblematisch. Reine Code-Organisation, keine Verhaltensänderung. **Vom Betreiber in Expo Go bestätigt.** Damit ist Fund 5 abgeschlossen. Laut ADR-Reihenfolge (Entscheidung 4) als Nächstes: Fund 6/7 (Training-/Kalorien-Fassaden), zuletzt Fund 8 (Truth Engine). Seit dem vorigen Feature-Snapshot (v107) davor neu dazugekommen (letzte Phase vor dem Kurswechsel): **Phase-Zero-Hinweis entfernt (PR #49)** — Betreiber-Vorgabe: der Nutzer darf nicht wissen, dass er in einer Baseline-/Beobachtungsphase steckt (Observer-Effekt, verfälscht das Tracking) — die Karte mit Titel/Beschreibung/Tage-Countdown ist komplett aus `MasterScreen` raus; die BMI-Text-Logik (`calculator.ts`) bleibt bestehen und getestet, nur ungenutzt. **Glas-Schwebe-Panel (PR #50)** — nach Betreiber-Referenzbild: neue Komponente `GlassSummaryPanel.tsx` bündelt Heute/Werkzeuge/Fokus/Ernährung/Manifest in einem gemeinsamen, seitlich scrollbaren Glas-Panel (Punkte-Raster für Tally-Werte, SVG-Kreisring "Phase X/4" für den Fokus, alles echte Daten – kein erfundenes "Ziel von X"); nutzt dieselben Liquid-Glass-Tokens wie `PremiumCard`, zeigt sich damit automatisch im "Frost White"-Look, sobald der Nutzer im Light-Stage ist (DesignLevel `phaseZero`, war schon in `theme.ts` angelegt). Ergänzt die bestehende Karten-Leiste (bleibt für volle Navigation), sitzt als abschließende Funktions-Leiste am Ende des Screens; Tipp-des-Tages- und Inspirations-Impuls-Karten dafür entfernt (Betreiber-Vorgabe: Screen soll clean bleiben). **Core-Bar-Feinschliff (PR #52)** — Betreiber-Feedback per annotiertem Screenshot: bei niedrigem Fortschritt (z. B. 20%) lag die leuchtende Treibstoff-Vorderkante direkt hinter dem Λ-Anker und wirkte wie eine unbeabsichtigte Umrandung ums Icon; die Füllung + der Licht-Sweep beginnen jetzt mit 24px Abstand NACH dem Icon-Slot (per DOM-Bounding-Box-Inspektion in einer lokalen Expo-Web-Vorschau verifiziert, da Screenshot-Vergleiche bei der geringkontrastigen Glas-Tönung optisch täuschen). Zusätzlich: "20% LIGHT"-Label jetzt in Modus-Akzentfarbe statt Grau; der Λ-Anker wechselt bei jedem App-Start zwischen "Λ" und "V" (`sl_anchor_glyph`); "Übersicht"-Titel aus `MasterScreen` entfernt; Shadow-Countdown-Label zeigt nur noch "VΛAΛV" ohne das Wort "COUNTDOWN". **Shadow-Bogen-Panel (PR #54)** — Betreiber-Vorgabe nach Foto-Mockup-Abstimmung: "nur Shadow-Mode-relevante Sachen" – Mahlzeit, Training, Gewicht, Schlaf jetzt in einem gemeinsamen, gewölbten Glas-Rahmen (`ShadowBogenPanel.tsx`, SVG-Bogen-Silhouette mit hellem Lichtrand + Glow) statt einzeln wischbarer Karten; Mahlzeit/Training bleiben Toggles (dieselbe `toggleBlock()`-Mechanik), Gewicht/Schlaf zeigen an fälligen Tagen ein kompaktes Eingabefeld, sonst den letzten Stand. Schlaf ist neu in Shadow Mode (tägliche Stundenzahl über die bestehende `healthMetrics.ts`-Speicherquelle, keine zweite Schlaf-Wahrheit); `SHADOW_BLOCK_LABEL.shadowFood` von "Ernährung" auf "Mahlzeit" umbenannt. **Bogen-Touch-Fix + Feinschliff (PR #56)** — Betreiber-Feedback per Live-Screenshot (v111 bereits im Feld getestet): das Antippen der Bogen-Spalten reagierte auf echten Geräten gar nicht — Ursache war das dekorative SVG (Bogen-Silhouette) ohne `pointerEvents="none"`, das die Touches abfing, bevor sie die Pressable-Spalten erreichten; jetzt gefixt und per Web-Vorschau verifiziert (Tap auf "Mahlzeit" schaltet sichtbar auf "Erledigt"). Der Bogen hatte außerdem nur oben/unten einen Lichtrand-Stroke, links/rechts fehlte der Rand komplett — wirkte wie eine freischwebende, losgelöste Linie; jetzt ein durchgehender Rand über die ganze Silhouette. Versions-Wasserzeichen von "unter der Core Bar, rechtsbündig" zu "unten am Bildschirm, mittig" verschoben. Core-Bar-"Next-Stage"-Label (z. B. "LIGHT" während man noch in Shadow ist) zeigt jetzt die Akzentfarbe des NÄCHSTEN Stage (`STAGE_LEVEL` aus `ThemeContext.tsx` exportiert) statt der aktuellen — vorher in Shadow unsichtbar, da Shadows Akzent ebenfalls Weiß ist wie die Prozentzahl daneben. **DEV: Modus überspringen (PR #58)** — Betreiber-Vorgabe: ein Button direkt auf dem `MasterScreen` (unter dem Modus-Chip), um sofort zum nächsten Modus zu springen statt die reale Wartezeit (14 Tage Shadow, `DAYS_PER_STAGE` sonst) abzuwarten — nutzt denselben `setStage()` wie der bestehende Dev-Schalter in Werkzeuge, nur ohne dorthin navigieren zu müssen; kein Button mehr ab Master. Zusätzlich: aktuelle Handoff-Datei sowie eine bereichsweise QA-Checkliste (Onboarding/Core Bar/Shadow/Light+/Wasserzeichen/24 Werkzeuge/restliche Screens/Allgemein, je mit Notiz-Zeile) an den Betreiber zum iPad-Review verschickt. Versions-Wasserzeichen-Zähler sechsmal nachgezogen (PR #48 v104→v107, PR #51 v107→v109, PR #53 v109→v110, PR #55 v110→v111, PR #57 v111→v112, PR #59 v112→v113). Der frühere „Session 2 / 65 Tests"-Block ist damit mehrfach überholt — der reale Stand ist deutlich weiter.
>
> **Offene Design-Explorationen (noch kein Code):** Betreiber hat ein "Hologramm"-Feature für das Glas-Schwebe-Panel angefragt (Scan-Sweep + Rasterlinien-Flackern beim Wischen, evtl. Partikel-Ring statt SVG-Kreis für die Fokus-Phase) sowie eine "Kreis als Maus/Joystick"-Navigationsidee — beide bewusst noch nicht gebaut, erst Mockup-Fotos zur Abstimmung verschickt (Kreis-Idee: Wiedervorlage 2026-07-25).
...
*   **Logik-Kern (`src/logic/`, 48 Module, alle mit Tests):** `calculator.ts`, `timeline.ts`, `dayplan.ts`, `dayArchitecture.ts`, `discipline.ts`, `tracking.ts`, `weekPlan.ts`, `sportplan.ts`, `macroPlanning.ts`, `mealtracking.ts`, `nutrition`, `supplements.ts`, `money.ts`, `medals.ts`, `confession.ts`, `identity.ts`, `focus.ts`, `hybridRouting.ts`, `bloodwork.ts`, `findings.ts`, `analytics.ts`, `monitoring.ts`, `injuryHub.ts`, `tabuBoerse.ts`, `ramadanMode.ts`, `arcMode.ts`, `bodyAtlas.ts`, `drivetrain.ts`, `gearing.ts`, `knowledge.ts`, `coinBook.ts`, `wallpaperBar.ts`, `resilience.ts`, `biometricResilience.ts`, `journal.ts`, `checkins.ts`, `regressionWatch.ts`, `survivalMode.ts`, `shadowOnboarding.ts`, u. a.
*   **Screens (`src/screens/`, 35):** u. a. `MasterScreen`, `DayScreen`, `DayArchitectureScreen`, `WeeklyPlanScreen`, `ToolsScreen`, `NutritionScreen`, `StackScreen`, `ProductsScreen`, `ShopScreen`, `TabuScreen`, `MoneyScreen`, `BloodworkScreen`, `FindingsScreen`, `BodyAtlasScreen`, `BodyScreen`, `RecoveryModeScreen`, `RamadanModeScreen`, `MonitoringScreen`, `InjuryHubScreen`, `AnalyticsScreen`, `MedalsScreen`, `ManifestScreen`, `QuizScreen`, `FocusScreen`, `GearingScreen`, `DrivetrainScreen`, `CoinBookScreen`, `JournalScreen`, `WallpaperBarScreen`, `InventoryScreen`, `GearSetupScreen`, `RidePerformanceScreen`, `SurvivalModeScreen`, `onboarding/OnboardingFlow`.
...
6.  **Setze vor Launch die DEV-Flags zurück:** `DAYS_PER_STAGE` (mobile) auf Produktionswert, `PREVIEW_UNLOCK_ALL = false` (PWA), `PREVIEW_UNLOCK_ALL_FLOWERS = false` (mobile, `coinBook.ts` — Betreiber-Vorschau vom 2026-07-11).
...
9b. **Wiedervorlage 2026-07-25 (Betreiber-Anweisung vom 2026-07-11):** Monetarisierungs-Ideen #172–#177 (Abschnitt 16a) erneut mit dem Betreiber durchgehen — Stand `vaaav-backend`, Nutzerzahlen, Priorisierung. Reminder in der Session ist gesetzt.
10. ~~**Themes/Vibes-Ausbau (#100)**~~ ✅ ~~**Budget-Planning (#130)**~~ ✅ (PR #9) ~~**Fokus-Matrix (#132)**~~ ✅ ~~**Hybrid-Routing (#140)**~~ ✅ ~~**Degradierung (#26/#169)**~~ ✅ ~~**Financial-Hub (#127)**~~ ✅ ~~**Eternity Endgame (#120)**~~ ✅ ~~**Resilienz-Engine (#136)**~~ ✅ (PR #10) — die ersten sieben waren reine Doku-Drift (10 Korrekturen in dieser Session, siehe Kapitel 0.3), #136 ist ein echtes neues Feature. **Universelle Weisheits-Datenbank (#137/#138)** bleibt der einzige offene Punkt aus dieser Gruppe — braucht reale Koran-Zitate, **bewusst nicht von der KI geraten/erfunden**, wartet auf Betreiber-Input (Quellenangabe, Auswahl der Verse). **Vor jedem weiteren Feature: kurz den Code prüfen, ob es nicht schon existiert** — die Drift-Quote lag bei ~10 von 171 Punkten.
