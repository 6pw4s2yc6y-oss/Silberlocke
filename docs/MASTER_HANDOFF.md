...
> Stand 2026-07-11 (Nacht): **451/451 Logik-Tests grün**, 48 Logik-Module (`src/logic/`), 35 Screens (`src/screens/`), Versions-Wasserzeichen bei **v107**. Seit dem Abend-Snapshot (v104) neu dazugekommen: **Shadow-Tag-Fix (PR #46)** — "Dein Tag" führte nach der ersten Drip-Frage noch auf den vollen, leeren `DayScreen`; jetzt direkt als 3-Slide-Karussell auf dem `MasterScreen` (Ernährung/Training als Toggle-Karten über `ShadowTrackerCard`, Gewicht als dritter Slide), kein Navigieren mehr nötig. **Mock-Daten-Aufräumen (PR #47)** — nach Betreiber-Feedback ("Daten wirken gekoppelt/vorausgefüllt"): `MasterScreen`s "Dein Körper"/"Werkzeuge"-Karten zeigten feste erfundene Mock-Werte (`mockDashboard`) statt echter Daten für jeden Nutzer — jetzt echte letzte Wiegung aus `sl_health_metrics` bzw. echte Werkzeug-Gesamtzahl; `WeeklyPlanScreen`s Default-Trainingsrhythmus sah wie ein bereits gespeicherter Plan aus — jetzt ein "Vorschlag, noch nicht gespeichert"-Hinweis, solange nichts editiert wurde. Zusätzlich in derselben PR: Shadow-Countdown zeigt jetzt durchlaufende Stunden statt Tage+Stunden (wirkt dringlicher) und in Danger-Rot statt neutral; Versions-Wasserzeichen sitzt jetzt unter statt über der Core Bar und leuchtet in einem durchlaufenden RGB-Farbverlauf. **Geprüft und bereits korrekt** (kein Fix nötig): Stack/Nahrung starten genuin leer, `sl_progress`/Tages-Log ebenfalls — falls alte Testdaten (z. B. abgehakte Blöcke) auf einem Gerät nach einem "frischen" Shadow-Start weiter auftauchen, liegt das an nicht zurückgesetztem Storage aus früheren Tests (Werkzeuge → Reset setzt wirklich alles zurück), nicht an einem Code-Fehler. Versions-Wasserzeichen-Zähler nachgezogen (PR #48, v104→v107, hatte 3 Merges lang stillgestanden). Der frühere „Session 2 / 65 Tests"-Block ist damit mehrfach überholt — der reale Stand ist deutlich weiter.
